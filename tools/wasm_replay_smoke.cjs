...
function readStageNote() {
  try {
    const length = Module._tandem_stage_len ? Module._tandem_stage_len() : 0;
    if (!length) return '';
    const pointer = Module._tandem_stage_ptr();
...
function readRuntimeError() {
  const length = Module._tandem_error_len();
  if (length) {
    const pointer = Module._tandem_error_ptr();
    return Buffer.from(Module.HEAPU8.subarray(pointer, pointer + length)).toString('utf8');
  }
  const stage = readStageNote();
  return stage
...
function run() {
  ...
  const loaded = Module._tandem_load_replay(pointer, bytes.length);
  Module._free(pointer);
  if (loaded !== 1) {
    console.error('FAIL: tandem_load_replay rejected ' + path.basename(replayPath) +
      '\n' + readRuntimeError());
    process.exit(1);
  }
  if (Module._tandem_packet_len() <= 0) {
    console.error('FAIL: first frame produced an empty packet');
    process.exit(1);
  }
  if (Module._tandem_mismatch_tick() !== -1) {
    console.error('FAIL: replay hash mismatch at tick ' + Module._tandem_mismatch_tick() +
      ' — the wasm sim diverged from the recording');
    process.exit(1);
  }
  let packetBytes = 0;
  for (let i = 0; i < frameBudget; i++) {
    if (Module._tandem_frame() !== 1) {
      console.error('FAIL: tandem_frame died at frame ' + i + '\n' + readRuntimeError());
      process.exit(1);
    }
    packetBytes += Module._tandem_packet_len();
  }
  if (Module._tandem_mismatch_tick() !== -1) {
    console.error('FAIL: replay hash mismatch at tick ' + Module._tandem_mismatch_tick() +
...
const bundlePath = path.join(distDir, 'tandem_replay.js');
