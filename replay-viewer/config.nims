...
# use-after-free fixed in tandem_replay.nim (emscripten_exit_with_live_runtime)
...
# tandem_error_len(). Aborting keeps linear memory intact, and the page reads
# tandem_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "tandem_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_tandem_load_replay,_tandem_frame,_tandem_input,_tandem_packet_ptr,_tandem_packet_len,_tandem_mismatch_tick,_tandem_error_ptr,_tandem_error_len,_tandem_stage_ptr,_tandem_stage_len
  """).replace("\n", " ")
)
