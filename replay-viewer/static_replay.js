(function () {
  ...
  function showFailure(error) {
    ...
    failed = true;
    console.error(error);
    // TANDEM addition to the inherited coworld-ctf shell: publish the failure
    // as a machine-readable attribute BEFORE the #status line is rendered, so
    // tools/ci/viewer_smoke.mjs can fail fast instead of waiting out its
    // timeout on a viewer that has already given up.
    try {
      document.documentElement.setAttribute(
        'data-replay-error', String(error && error.message || error));
    } catch (ignored) { /* attribute setting cannot be allowed to throw */ }
  ...
  function createCore(config) {
    ...
    function start() {
      ...
      try {
        worker = new Worker(workerUrl, { name: 'tandem-static-replay' });
