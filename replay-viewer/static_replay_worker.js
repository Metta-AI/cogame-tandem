...
function stageNote() {
  ...
  try {
    var length = Module._tandem_stage_len ? Module._tandem_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._tandem_stage_ptr();
...
function runtimeError() {
  var length = Module._tandem_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._tandem_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function ingestPacket() {
  var length = Module._tandem_packet_len();
  if (!length) throw new Error('Replay runtime produced an empty frame');
  var pointer = Module._tandem_packet_ptr();
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._tandem_input(pointer, length);
  });
}
...
async function start() {
  ...
  try {
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._tandem_load_replay(pointer, length);
    });
    ...
    postMessage({
      type: 'loaded',
      mismatchTick: Module._tandem_mismatch_tick()
    });
  } catch (error) {
    reportFailure(error);
  }
...
function advance(frames) {
  ...
  try {
    ...
    for (var i = 0; i < count; i++) {
      if (Module._tandem_frame() < 0) throw new Error(runtimeError());
      ingestPacket();
    }
    postMessage({
      type: 'advanced',
...
importScripts('./wire_constants.js', './broadcast_core.js', './tandem_replay.js');
